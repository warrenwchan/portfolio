  ...
  return (
    <Motion
      className={`grid grid-cols-2 ${
        imageData.length % 3 == 0 ? "md:grid-cols-3" : "md:grid-cols-2"
      } gap-2`}
    >
      {imageData.map((image, index) => {
        return (
          <div
            className={`col-span-1 row-span-1 last:odd:col-span-2 ${
              imageData.length % 2 !== 0 && imageData.length % 3 !== 0
                ? "md:last:odd:col-span-2"
                : "md:last:odd:col-span-1"
            } hover:cursor-pointer`}
            key={index}
          >
            <div className="relative w-full after:block after:pb-[69.81%] after:content-['']">
